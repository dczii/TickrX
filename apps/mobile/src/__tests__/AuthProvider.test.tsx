...
function Consumer() {
  ...
  return <Text>{session ? `user:${(session as { user: { id: string } }).user.id}` : 'signed-out'}</Text>;
}
...
describe('AuthProvider', () => {
  it('restores a persisted session on mount (app restart)', async () => {
    ...
    await waitFor(() =>
      expect(screen.getByText('user:persisted-user')).toBeTruthy()
    );
  });
  ...
  it('updates when auth state changes', async () => {
    ...
    await waitFor(() =>
      expect(screen.getByText('user:fresh-user')).toBeTruthy()
    );
  });
